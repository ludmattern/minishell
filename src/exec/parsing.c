...
t_io_node* create_io_node(t_io_type type, const char* value) {
    ...
    io_node->expanded_value = NULL; // Assuming expansion is handled elsewhere
    io_node->here_doc = 0;
    io_node->prev = NULL;
    io_node->next = NULL;
    return io_node;
...
t_node* create_ast() {
    ...
    t_node* echo_success_cmd = create_cmd_node("echo", NULL);
    ...
    echo_failure_cmd->expanded_args = realloc(echo_failure_cmd->expanded_args, 3 * sizeof(char*));
